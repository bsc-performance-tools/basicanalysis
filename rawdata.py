...
raw_data_doc = OrderedDict([('runtime', 'Runtime (us)'),
                            ...
                            ('frequency', 'Average frequency (cycles/us)'),
...
def parse_average_frequency_histogram(path):
    """
    Compute the time-weighted average frequency from the
    Cycles-per-us 2D histogram generated by frequency_avg_value.cfg.

    Histogram columns have the form:
    ...
    The Total row contains the accumulated time in each frequency bin.

    Returns:
        Average frequency in cycles/us, or None when no valid
        ...
        for raw_line in f:
            ...
            if not bin_midpoints:
                ...
                if candidate_midpoints:
                    bin_midpoints = candidate_midpoints
                    continue

            # --------------------------------------------------
            # Total accumulated time per frequency bin
            # --------------------------------------------------
            if key == 'Total':
    ...
    if not bin_midpoints or not total_times:
        return None
    ...
    for index in range(count):
        ...
        frequency = bin_midpoints[index]
        duration = total_times[index]
...
def parse_io_operations_stats(path):
    """Parse per-operation I/O statistics generated by Paraver.

    The input CSV contains one column per I/O operation and summary rows
    including Total, Average, and Maximum.

    Returns:
        dict: Statistics indexed by I/O operation, for example:
            {
                'read': {
                    'tot': 0.0,
                    'avg': 0.0,
    ...
    with open(path) as f:
        ...
        for line in f:
            ...
            if row_name not in row_to_key:
                continue

            stat_key = row_to_key[row_name]

            for operation, value in zip(operations, fields[1:]):
                if value.strip():
                    stats[operation][stat_key] = float(value)
...
def create_io_raw_data():
    """Create raw I/O characterization data for a single trace."""
    return {
        'operations': {
            operation: {
                'time': {
...
def init_cfgs():
    ...
    cfgs['frequency_avg'] = os.path.join(cfgs['root_dir'],'frequency_avg_value.cfg')
...
def process_one_trace(trace, trace_process_count, trace_task_per_node_value,
    ...
    cmd_base.extend([cfgs['frequency_avg'],trace_name + '.frequency_avg.stats.csv'])
    ...
    # Average frequency
    frequency_path = trace_name + '.frequency_avg.stats.csv'

    if os.path.exists(frequency_path):

        frequency_avg = parse_average_frequency_histogram(
            frequency_path
        )
    ...
    if (
        cmdl_args.debug
        and trace_raw_data['frequency'] != 'Non-Avail'
    ):
        print(
            '==DEBUG== Average frequency: '
            '{:.2f} cycles/us ({:.4f} GHz)'.format(
                float(trace_raw_data['frequency']),
                float(trace_raw_data['frequency']) / 1000.0
            )
        )
    ...
    if os.path.exists(trace_name + '.io_calls_count.stats.csv'):
        ...
        for operation in IO_OPERATIONS:
            if operation in io_count_stats:
                trace_io_data['operations'][operation]['count'].update(
                    io_count_stats[operation]
                )
    ...
    if os.path.exists(trace_name + '.io_calls_time.stats.csv'):
        ...
        for operation in IO_OPERATIONS:
            if operation in io_time_stats:
                trace_io_data['operations'][operation]['time'].update(
    ...
    move_files(trace_name + '.frequency_avg.stats.csv', local_path_dest, cmdl_args)
