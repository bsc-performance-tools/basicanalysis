...
def _build_hybrid_xaxis_label(
    ...
    """
    Build a model-aware description of the configuration notation
    used in hybrid plot x-axis tick labels.

    [Trace ID] is included only when trace IDs are actually present
    in the displayed configuration labels.
    """

    # Detect whether the configuration labels were disambiguated
    # with trace IDs such as:
    #
    #     64 (8x8) [1]
    #     64 (8x8) [2]
    #
    # For MPI+GPU, labels may also contain [4D], [8D], etc.,
    # so checking merely for square brackets is not sufficient.
    show_trace_id = any(
        label.endswith(
            '[{}]'.format(index + 1)
        )
        for index, label in enumerate(configuration_labels)
    )
    ...
    if all(
            trace_mode[trace].startswith('Detailed+MPI+')
            for trace in trace_list
    ):
        runtime_name = trace_mode[trace_list[0]][
            len('Detailed+MPI+'):
        ]

        if runtime_name == 'OpenMP':
            return (
                'Parallel units\n'
                'Configuration: '
                '(MPI ranks × threads/rank)'
                + trace_id_suffix
            )
...
def compute_model_factors(raw_data, trace_list, trace_processes, trace_mode, list_mpi_procs_count, cmdl_args):
    ...
    for trace in trace_list:
        ...
        try:
            ...
            # File I/O is performed on the Host. For hybrid executions,
            # accelerator execution contexts must therefore not contribute
            # to the aggregate execution capacity used by the I/O metrics.
            host_parallel_units = int(
                raw_data['count_host_threads'][trace]
            )

            if host_parallel_units <= 0:
                host_parallel_units = int(total_procs)
        ...
        #
        # Internal quantities used for diagnostics and warning
        # thresholds. They are not public I/O metrics.
        # --------------------------------------------------

        if (
            runtime > 0.0
            and host_parallel_units > 0
            and trace_mode[trace][:5] != 'Burst'
        ):

            other_metrics['io_mpiio'][trace] = (
                mpi_io_tot
                / (runtime * host_parallel_units)
                * 100.0
            )
        ...
        if (
            ...
            try:
                mpi_io_avg = float(
                    raw_data['mpiio_avg'][trace]
                )

                mpi_io_max = float(
                    raw_data['mpiio_max'][trace]
                )
        ...
        if (
            ...
            try:
                posix_io_avg = float(
                    raw_data['io_avg'][trace]
                )

                posix_io_max = float(
                    raw_data['io_max'][trace]
                )
        ...
        #
        # Tracer flushing is intentionally excluded because it is
        # instrumentation overhead rather than application File I/O.
        # --------------------------------------------------
  ...
  # ------->  HOST metrics
    ...
    # Basic scalability factors - Computational
        ...
        # Experimental IPC Scalability including serial I/O
        try:
            if not ENABLE_IO_AWARE_METRICS:
        ...
        # Experimental Frequency Scalability including serial I/O
        try:
            if not ENABLE_IO_AWARE_METRICS:
    ...
    # Instruction Scalability
        ...
        # Experimental Instruction Scalability including serial I/O
        try:
            if not ENABLE_IO_AWARE_METRICS:
...
def print_other_metrics_table(
    ...
    # --------------------------------------------------
    # Build hybrid configuration labels
    # --------------------------------------------------
    ...
    for trace in trace_list:
        ...
        # I/O exists when at least one API-specific
        # efficiency metric is available.
        if (
            other_metrics['mpi_io_eff'][trace] != 'Non-Avail'
        ...
        # Keep the previous 5% threshold only as an
        # informational indication of significant I/O.
        if (
...
def print_other_metrics_csv(other_metrics, trace_list, trace_processes):
    ...
    print(
        'General performance indicators, I/O metrics, and '
        'flushing information written to ' + file_path
    )
...
def plots_speedup_matplot(
    ...
    Numeric x coordinates are kept independent from the displayed
    configuration labels. This allows traces with the same number
    of parallel units to remain individually visible.
    """
    ...
    # --------------------------------------------------
    # Canonical configuration labels
    # --------------------------------------------------
    ...
    x_axis_label = _build_hybrid_xaxis_label(
        trace_list,
        trace_mode,
        configuration_labels,
    )
    ...
    # Keep plotting coordinates separate from labels.
    #
    # If several traces have the same number of parallel units,
    # move subsequent points slightly to the right so that they
    ...
    for trace in trace_list:
        ...
        if previous_units == parallel_units:
            repeated_count += 1
    ...
    # Leave enough vertical space for measured-value annotations.
    finite_speedup = [
        value
    ...
    finite_ideal_speedup = [
        value
        for value in ideal_speedup
        if not pd.isna(value)
    ]

    max_speedup = max(
        finite_speedup + finite_ideal_speedup
    ) if (finite_speedup or finite_ideal_speedup) else 1.0
    ...
    fig.savefig(
        'efficiency_matplot.png',
        dpi=400,
        bbox_inches='tight'
    )

    fig.savefig(
        'efficiency_matplot.pdf',
        bbox_inches='tight'
    )
